...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery, pa_g_nodes):
	...
	q_table = np.random.rand(pa_size * (ep_len+1),9) * 0.001 - 0.001  # of states x # of actions
	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			if len(possible_next_states_copy[agent_s]) == 0 and hit == 1:
				agent_s = agent_upt.index(pa2ts[agent_s])	
			...
			current_q = q_table[prev_s * (ep_len+1) + t_ep, intended_action]
			max_future_q = np.amax(q_table[agent_s * (ep_len+1) + t_ep+1, :])                       # Find the max future q 
			...
			q_table[prev_s * (ep_len+1) + t_ep, intended_action] = new_q                            # Update the table
			ep_rew += rew_obs
