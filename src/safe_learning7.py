...
if __name__ == '__main__':
	...
	LEARN_FLAG = False # If true learn a new Q table, if False load the previously found one
	...
	if LEARN_FLAG:
		...
		if thr_fun < Pr_des:
			...
			for test_n in range(1):
				opt_pol = Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states, pick_up, delivery,  pick_ups, deliveries,test_n)
				print("Total run time : %s seconds" % (time.time() - start_time))

			##### Visualize ######
			visualization(m, n, init_states, obstacles, pick_up_state[0], delivery_state[0], rewards, opt_pol)
	else:
		agent_s_phy = 45 # initial physical starting state
		agent_upt_i = []
		agent_upt = []
		for i in range(len(pa[0].g.nodes())):
			if pa[0].g.nodes()[i][1] == 0 or str(pa[0].g.nodes()[i][0]) == 'r'+str(pick_up[0]):
				agent_upt_i.append(pa2ts[0][i])
			else:
				agent_upt_i.append([])
		agent_upt.append(agent_upt_i)
		agent_s = agent_upt[0].index(pa2ts[0][agent_s_phy])

		Q_TABLE = np.load("Converged_Q_TABLE.npy", allow_pickle=True)
		tot_und_rew = 0
		tot_disc_rew = 0
		for t_ep in range(ep_len):
			rew_obs = rewards_pa[0][agent_s]
			action = np.argmax(Q_TABLE[agent_s  * ep_len + (ep_len-t_ep-1),:])
			possible_current = []
			matches = [i==agent_s for i in pa_s[0]]
			ind = 0
			for check in matches:
				if check:
					possible_current.append(ind)
				ind += 1
			for  ix in possible_current:
				if act_num[0][ix] == action:
					agent_s = pa_t[0][ix]
					break
			
			#next_ind = possible_acts[0][agent_s].index(action)
			#agent_s = possible_next_states[0][agent_s][next_ind]
			tot_und_rew += rew_obs
			tot_disc_rew += rew_obs * (DISCOUNT ** t_ep)
		print('Total Undiscounted Reward after learned policy = '+str(tot_und_rew))
		print('Total Discounted Reward after learned policy = '+str(tot_disc_rew))
