...
def get_possible_actions(energy_pa, pa2ts, pa_s, pa_t):
	...
	return possible_acts, possible_next_states


def Q_Learning(N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, possible_acts, possible_next_states,  pick_up):
	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			if hit == 0:       
				not_possible_index = []         
				for i in range(len(possible_next_states_copy[agent_s])):
					if energy_pa[possible_next_states_copy[agent_s][i]] > ep_len - t_ep - 1: # ?
						not_possible_index.append(i)

				# if len(not_possible_index) > 0:
				for ind in sorted(not_possible_index, reverse=True):
					if len(possible_acts_copy[agent_s]) == 0:
						break
			...
			action    = possible_acts_copy[agent_s][next_ind]               # Taking the action
...
if __name__ == '__main__':
	...
	[i_s, pa, pa_s, pa_t, pa2ts, energy_pa, rewards_pa, pick_up] = prep_for_learning(ep_len, m, n, h, init_states, obstacles, pick_up_state[0], delivery_state[0], rewards, rew_val, custom_flag, custom_task)
	[possible_acts, possible_next_states] = get_possible_actions(energy_pa, pa2ts, pa_s, pa_t)
	...
	DISCOUNT   = 0.95
	EPS_DECAY  = 1  #0.99989
	epsilon    = 0.1# 0.95 exploration trade-off
	eps_unc    = 0.10 # Uncertainity in actions
	Pr_des     = 0.70

	# Call the Q_Learning Function
	opt_pol = Q_Learning(N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, possible_acts, possible_next_states, pick_up)
