...
def Q_Learning(rnd, repeat, Pr_des, eps_unc, eps_unc_learning, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts_not_pruned, possible_acts_pruned, possible_next_states_not_pruned,possible_next_states_pruned, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size):
	...
	indices=[0,1,2, int(N_EPISODES/2), int(N_EPISODES/2+1), int(N_EPISODES/2+2),N_EPISODES-3,N_EPISODES-2,N_EPISODES-1]#
	optimal_policy_pas = []
	...
	filename = 'model_based_GNC_random' + str(int(rnd)) +'_' +str(n_samples) +'_trial' + str(int(repeat)) +'.xls'
	filename = filename
	wb.save(filename) 
...
if __name__ == '__main__':
	...
	for rnd in range(0,1):
		# pick_up_states = [[[(0,  1, 0)],[(5,  5, 0)]], [[(6,  6, 0)],[(1,  1, 0)]], [[(2,  1, 0)],[(2,  5, 0)]], [[(6,  3, 0)],[(0,  1, 0)]], [[(3,  0, 0)],[(3,  2, 0)]],\
		#                   [[(5,  2, 0)],[(1,  6, 0)]], [[(6,  1, 0)],[(6,  6, 0)]], [[(5,  4, 0)],[(3,  6, 0)]], [[(3,  5, 0)],[(4,  3, 0)]], [[(2,  5, 0)],[(4,  5, 0)]]] 
		# delivery_states = [[[(6,  2, 0)],[(1, 5 , 0)]], [[(6,  1, 0)],[(4, 4, 0)]], [[(3,  5, 0)],[(4, 6 , 0)]], [[(4,  6, 0)],[(3, 2 , 0)]], [[(4,  3, 0)],[(1, 3 , 0)]],\
		#                    [[(4,  3, 0)],[(3,  0, 0)]], [[(3,  2, 0)],[(0,  2, 0)]], [[(6,  2, 0)],[(2,  1, 0)]], [[(5,  6, 0)],[(1,  5, 0)]], [[(2,  6, 0)],[(4,  4, 0)]]]
		pick_up_states = [[[(6,  6, 0)],[(1,  1, 0)]]] 
		delivery_states =[[[(6,  1, 0)],[(4,  4, 0)]]]
		pick_up_state = pick_up_states[rnd]
		delivery_state = delivery_states[rnd]                                        
		...
		n_samples_all = [10]#[0, 5, 10, 20, 50] # Running the algorithm for different model based samples, 0 for model free learning
			...
			if thr_fun < Pr_des:
				...
				for repeat in range(1):
					for n_samples in n_samples_all: #for test_n in range(1):
						test_n = 0
						opt_pol = Q_Learning(rnd, repeat, Pr_des, eps_unc, eps_unc_learning, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts_not_pruned, possible_acts_pruned, possible_next_states_not_pruned,possible_next_states_pruned, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size)
