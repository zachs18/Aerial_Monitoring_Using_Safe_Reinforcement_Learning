...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery,  pick_ups, deliveries, test_n, n_samples):
		...
		for t_ep in range(ep_len):
			...
			if hit[which_pd] == 0: 
				...
				for j in range(len(possible_next_states_copy[agent_s[which_pd]])):
					...
					if thr_fun < Pr_des and i_max > 0: #energy_pa[possible_next_states_copy[agent_s][j]] > k_ep-2: # 
			...
			prev_state = real_agent_s
			real_action = possible_acts_copy[real_agent_s][next_ind]
			if np.random.uniform() < eps_unc - 0.08:
			...
			all_samples[which_pd].append([prev_state, real_action, rew_obs, agent_s[which_pd], t_ep]) # S,A,R,S' and time
	...
	filename = 'model_based_v3_time_included' +str(n_samples) +'.xls'
	filename = filename
	wb.save(filename) 
	...
	if LEARN_FLAG:
		...
		if thr_fun < Pr_des:
			...
			for n_samples in n_samples_all: #for test_n in range(1):
				test_n = 0
				opt_pol = Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples)
