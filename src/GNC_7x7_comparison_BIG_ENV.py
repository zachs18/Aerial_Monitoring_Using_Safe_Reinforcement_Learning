...
def Q_Learning(Pr_des, eps_unc, eps_unc_learning, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts_not_pruned, possible_acts_pruned, possible_next_states_not_pruned,possible_next_states_pruned, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size):
	...
	print('Over estimated action uncertainity[%] = ' + str(eps_unc_learning*100))
	...
	filename = 'model_based_GNC_bigEnv_' +str(n_samples) +'.xls'
	filename = filename
	wb.save(filename) 
...
if __name__ == '__main__':
	...
	ep_len = 101 # Episode length
	m = 100       # of rows
	n = 100       # of columns  8
	h = 1       # height set to 1 for 2D case
	ts_size =m*n
	
	# Specify insitial states and obstacles (row,column,altitude/height)
	init_states    = [(22,22,0)]                                         # Specify initial states and obstacles (row,column,altitude/height)
	obstacles = [(2,3,0), (2,4,0), (3,3,0), (3,4,0)]
	
	# Specify pick-up and delivery locations
	pick_up_state = []
	delivery_state = []                                      
	pick_up_state.append([(2,18,0)])
	#pick_up_state.append([(32,8,0)])
	# pick_up_state.append([(96,59,0)]) 
	# pick_up_state.append([(35,48,0)])
	# pick_up_state.append([(57,1,0)])                                        
	# delivery_state.append([(10,67,0)])                                         
	# delivery_state.append([(51,3,0)])                                         
	# delivery_state.append([(0,60,0)])                                         
	delivery_state.append([(39,27,0)])
	#delivery_state.append([(49,39,0)])

	# Specify the reward locations, and reward uncertainty
	rewards   = [(4,0,0), (4,1,0), (5,0,0), (5,1,0), (44,0,0), (44,1,0), (45,0,0), (45,1,0), (4,40,0), (4,41,0), (5,40,0), (5,41,0),(44,40,0), (44,41,0), (45,40,0), (45,41,0)] # Reward 1 Reward Locations
	...
	eps_unc    = 0.00001 # Uncertainity in actions, real uncertainnity in MDP
	eps_unc_learning = 0.00002 # Overestimated uncertainity used in learning
	Pr_des     = 0.35 # Minimum desired probability of satisfaction
