...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery,  pick_ups, deliveries,test_n):
	...
	np.save("q_table_diff_perc.npy",q_table_diff_perc)
	np.save("Converged_Q_TABLE_base.npy",q_table[0])
...
if __name__ == '__main__':
	...
	if LEARN_FLAG:
		...
		Q_TABLE = np.load("Converged_Q_TABLE_base.npy", allow_pickle=True)
		avg_tot_und_rew = 0
		avg_tot_disc_rew = 0
		sample_size = 1000
		#
		...
		for i in range(len(pa[0].g.nodes())):
			if pa[0].g.nodes()[i][1] == 0 or str(pa[0].g.nodes()[i][0]) == 'r'+str(pick_up[0]):
				...
				agent_upt_i.append([])
			if pa[0].g.nodes()[i][1] == 0 and pa[0].g.nodes()[i][0] == 'Base1':
				starting_state = i
		agent_upt.append(agent_upt_i)
		#
		hit_raise = 0
		for k in range(sample_size):
			...
			for t_ep in range(ep_len):
				...
				if np.random.uniform() < eps_unc:
					[chosen_act, agent_s] = action_uncertainity(action, pa_s[0], pa_t[0], act_num[0], agent_s)
				else:
					possible_current = []
					...
					for  ix in possible_current:
						if act_num[0][ix] == action:
							agent_s = pa_t[0][ix]
							break
				if hit_flag == False and energy_pa[0][agent_s] == 0:
					hit_raise += 1
					hit_flag = True
				tot_und_rew += rew_obs
