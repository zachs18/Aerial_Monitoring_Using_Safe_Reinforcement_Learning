'''
.. module:: Safe RL
   Simulatenous Update as in paper
...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery,test_n):
	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			current_q = q_table[which_pd][real_agent_s, real_action]
			max_future_q = np.amax(q_table[which_pd][agent_s[which_pd], :])                                          # Find the max future q 	
			...
			q_table[which_pd][real_agent_s, real_action] = new_q 
