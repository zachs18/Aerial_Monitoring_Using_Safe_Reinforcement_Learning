...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts_not_pruned, possible_acts_pruned, possible_next_states_not_pruned,possible_next_states_pruned, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size):
	...
	for i in range(len(energy_pa)):
		for j in range(ep_len):
			...
			name_q    = "Env3_Converged_Q_TABLE_base_N" + str(n_samples) + '_task'+ str(i) + '_t' + str(j) + ".npy"
	...
	filename = 'model_based_env3' +str(n_samples) +'.xls'
	filename = filename
	wb.save(filename) 
...
if __name__ == '__main__':
	...
	custom_task = '[H^1 r35]^[0, 10] * [H^1 r5]^[0,10] * [H^1 Base1]^[0,10]' # 
	##### System Inputs for Data Prep. #####
	ep_len = 32 # Episode length
	m = 6       # of rows
	n = 6       # of columns  8
	h = 1       # height set to 1 for 2D case
	ts_size =m*n
	init_states    = [(0,0,0)]                                         # Specify initial states and obstacles (row,column,altitude/height)
	obstacles = [(3,5,0), (3,4,0),  (2,5,0),  (2,4,0)]    # Indexed from (0,0,0) which is the upper left corner at ground height
	pick_up_state = []
	delivery_state = []
	###pick_up_state.append([(7,1,0)])                                      # 
	###pick_up_state.append([(0,7,0)])                                         # 
	pick_up_state.append([(5,5,0)])                                         # 
	...
	delivery_state.append([(0,5,0)])                                         # Specify delivery states
	...
	LEARN_FLAG = False # False # If true learn a new Q table, if False load the previously found one
	sample_size = 1000
	N_EPISODES = 10000      # of episodes
	SHOW_EVERY = 500       # Print out the info at every ... episode
	LEARN_RATE = 0.1
	DISCOUNT   = 0.95
	EPS_DECAY  = 1#1  #0.99989
	epsilon    = 0.1# exploration trade-off
	eps_unc    = 0.15 # Uncertainity in actions
	Pr_des     = 0.7
	...
	for ind_p in range(len(pick_up_state)):
		for ind in range(len(delivery_state)):
			[i_s_i, pa_i, pa_s_i, pa_t_i, pa2ts_i, energy_pa_i, rewards_pa_i, pick_up_i, delivery_i,  pick_ups_i, deliveries_i, pa_g_nodes_i] = prep_for_learning(ep_len, m, n, h, init_states, obstacles, pick_up_state[ind_p], delivery_state[ind], rewards, rew_val, custom_flag, custom_task)
			i_s.append(i_s_i)
	...
	if LEARN_FLAG:
		...
		Q_TABLEs = []
		agent_upt = []
		n_samples = 20 # choose for which n_samples to run simulation
		for ind in range(len(energy_pa)):
			Q_TABLEs.append([])
			for j in range(ep_len):
				name_q    = "Env3_Converged_Q_TABLE_base_N" + str(n_samples) + '_task'+ str(ind) + '_t' + str(j) + ".npy"
