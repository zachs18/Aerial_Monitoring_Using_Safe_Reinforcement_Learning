...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size):
	...
	for episode in range(N_EPISODES):
		...
		disc_ep_rew = 0
		possible_next_states_copied = copy.deepcopy(possible_next_states[which_pd])
		possible_acts_copied = copy.deepcopy(possible_acts[which_pd]) 
		...
		for t_ep in range(ep_len):
			...
			possible_next_states_copy = possible_next_states_copied
			possible_acts_copy = possible_acts_copied 
			# print('step : ,', t_ep)
			...
			if hit[which_pd] == 0:	
				...
				if len(possible_next_states_copy[agent_s[which_pd]]) != 0:		
					for ind in sorted(not_possible_index, reverse=True):
						...
						possible_next_states_copy[agent_s[which_pd]].pop(ind)
						possible_acts_copy[agent_s[which_pd]].pop(ind)
...
if __name__ == '__main__':
	...
	eps_unc    = 0.150 # Uncertainity in actions
	Pr_des     = 0.7
