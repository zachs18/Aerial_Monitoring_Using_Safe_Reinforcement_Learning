...
def Q_Learning(repeat,Pr_des, eps_unc, eps_unc_learning, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts_not_pruned, possible_acts_pruned, possible_next_states_not_pruned,possible_next_states_pruned, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size):
	...
	filename = 'model_based_GNC_unc' + str(int(eps_unc_learning*100)) + '_samples' +str(n_samples) +'_trial' + str(int(repeat+1)) + '.xls'
	filename = filename
	wb.save(filename) 
...
if __name__ == '__main__':
	...
	obstacles = [(1,2,0), (1,3,0), (1,4,0), (3,2,0), (3,3,0), (3,4,0)]
	
	# Specify pick-up and delivery locations
	pick_up_state = []
	delivery_state = []                                      
	pick_up_state.append([(6,0,0)])                                        
	delivery_state.append([(6,6,0)])                                        

	# Specify the reward locations, and reward uncertainty
	rewards   = [(2,3,0)] # Reward 1 Reward Locations
	rewards2  = [(0,3,0)]                  # Reward 2 Rewards Locations
	...
	N_EPISODES = 25000      # of episodes
	SHOW_EVERY = 2500       # Print out the info at every ... episode
	LEARN_RATE = 0.1
	DISCOUNT   = 0.95
	EPS_DECAY  = 1 #0.99989
	epsilon    = 0.1# exploration trade-off
	eps_unc    = 0.05 # Uncertainity in actions, real uncertainnity in MDP	
	
	eps_unc_learnings = [0.18]
	for idx in range(len(eps_unc_learnings)):			
		...
		Pr_des     = 0.6 # Minimum desired probability of satisfaction

		n_samples_all = [10] # Running the algorithm for different model based samples, 0 for model free learning
