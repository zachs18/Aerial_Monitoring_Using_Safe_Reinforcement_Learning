...
def Q_Learning(Pr_des, eps_unc, eps_unc_learning, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts_not_pruned, possible_acts_pruned, possible_next_states_not_pruned,possible_next_states_pruned, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size):
	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			if episode > 1000: # First get some samples before starting the sampling
				# Sample n times
...
if __name__ == '__main__':
	...
	pick_up_state = []
	delivery_state = []                                      
	pick_up_state.append([(7,7,0)])
	pick_up_state.append([(5,5,0)])                                         
	delivery_state.append([(0,4,0)])                                         
	delivery_state.append([(7,0,0)])                                         

	# Specify the reward locations, and reward uncertainty
	rewards   = [(0,0,0), (0,1,0), (1,1,0), (0,2,0), (1,0,0), (1,2,0), (2,0,0),(2,1,0), (2,2,0)] # Reward 1 Reward Locations
	...
	Pr_des     = 0.90 # Minimum desired probability of satisfaction

	n_samples_all = [0, 10, 20] # Running the algorithm for different model based samples, 0 for model free learning
