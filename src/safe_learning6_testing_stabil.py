...
def prep_for_learning(ep_len, m, n, h, init_states, obstacles, pick_up_state, delivery_state, rewards, rew_val, custom_flag, custom_task):
	...
	tf  = str((ep_len-2)/3) # time bound
	if custom_flag == 1:
		phi = custom_task
	else:
		phi = '[H^1 r' + pick_up + ']^[0, ' +  tf + '] * [H^1 r' + delivery + ']^[0,' + tf + ']  * [H^1 Base1]^[0,' + tf + ']' # Construc the task according to pickup/delivery )^[0, ' + tf + ']'
...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery,test_n, m ,n):
	...
	for i in range(len(energy_pa)):		
		...
		q_table.append(np.random.rand(m*n,9) * 0.001 - 0.001)  # of states x # of actions
	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			if np.random.uniform() > epsilon:                              # Exploit
				possible_qs = q_table[which_pd][pa2ts[which_pd][agent_s[which_pd]], possible_acts_copy[agent_s[which_pd]]] # Possible Q values for each action
			...
			max_future_q = np.amax(q_table[which_pd][pa2ts[which_pd][agent_s[which_pd]], :])                                          # Find the max future q 	
			...
			for i in range(len(energy_pa)):
				if i != which_pd:
					...
					q_table[i][hypo_phy_agent, real_action] = new_q    # Update the table				
		...
		if (episode+1) % 10000 == 0:
			...
			for ind in range(len(energy_pa)):
				...
				s1.write(ind*N_EPISODES/10000+inx,1,len(ep_per_task[ind]))
...
if __name__ == '__main__':
	...
	custom_task = '[H^1 r56]^[0, 10] * [H^1 r46]^[0,10] * [H^1 Base1]^[0,10]' # An example '[H^1 r63]^[0, 45] * [H^1 r7]^[0,45] * [H^1 r0]^[0,45] * [H^1 r48]^[0,45] * [H^1 r55]^[0,45])^[0, 45]'
	##### System Inputs for Data Prep. #####
	ep_len = 41 # Episode length
	m = 9       # of rows
	n = 9       # of columns  8
	h = 1       # height set to 1 for 2D case
	init_states    = [(0,0,0)]                                         # Specify initial states and obstacles (row,column,altitude/height)
	obstacles = [(3,5,0), (3,6,0),  (2,5,0),  (2,6,0)]    # Indexed from (0,0,0) which is the upper left corner at ground height
	pick_up_state = []
	delivery_state = []
	...
	rewards   = [(6,0,0), (6,1,0), (5,0,0), (5,1,0)] # Rewarded states
	...
	for ind_p in range(len(pick_up_state)):
		for ind in range(len(delivery_state)):
			...
			# print("PA = ")
			#print(pa_i.g.nodes().index(('r140',0)))
	...
	if thr_fun < Pr_des:
		...
		for test_n in range(1):
			opt_pol = Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states, pick_up, delivery,test_n, m ,n)
