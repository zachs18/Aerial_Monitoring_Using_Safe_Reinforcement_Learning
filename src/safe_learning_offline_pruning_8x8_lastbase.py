...
def get_possible_actions(pa_g_nodes, energy_pa, pa2ts, pa_s, pa_t, ep_len, Pr_des, eps_unc, pick_up):
	...
	print(len(pa_g_nodes))
	# print(energy_pa)
	# for  ind, val in enumerate(pa_g_nodes):
	# 	print(val)
	# 	print(energy_pa[ind])
	# 	print('next')
	#Time Layers
...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts_not_pruned, possible_acts_pruned, possible_next_states_not_pruned,possible_next_states_pruned, pick_up, delivery,  pick_ups, deliveries, test_n, n_samples, ts_size):
	...
	for i in range(len(energy_pa)):
		for j in range(ep_len):
			...
			name_q    = "Env3_Converged_Q_TABLE__8x8" + str(n_samples) + '_task'+ str(i) + '_t' + str(j) + ".npy"
...
if __name__ == '__main__':
	...
	custom_task = '[H^1 r46]^[0,10] * ([H^1 r57]^[0, 10] | [H^1 r24]^[0, 10])  * [H^1 Base1]^[0,10]'# '[H^1 r46]^[0,10] * ([H^1 r57]^[0, 10] | [H^1 r24]^[0, 10])  & [H^1 Base1]^[27,32]' # 
	...
	obstacles = [(4,0,0), (5,0,0), (4,1,0), (5,1,0), (3,3,0), (4,3,0),\
	 (2,3,0), (3,4,0), (4,4,0), (2,4,0), (3,5,0), (4,5,0), (2,5,0)]
	pick_up_state = []
	delivery_state = []
	...
	# delivery_state.append([(7,1,0)])                                         # Specify delivery states
