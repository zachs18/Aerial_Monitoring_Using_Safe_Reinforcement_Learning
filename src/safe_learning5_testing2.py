...
def simul_upt(pa2ts, pa_s, pa_t, act_num, action, real_agent_s):
	current_agent_list = []
	next_agent_list = []
	agent_mdp = pa2ts[real_agent_s]
	for i, val in enumerate(pa_s):
		if pa2ts[val] == agent_mdp and act_num[i] == action:
			current_agent_list.append(val)
			next_agent_list.append(pa_t[i])

	return current_agent_list, next_agent_list


def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery,test_n ):
	wb = Workbook()
	sheet_name = 'Simulation' + str(test_n+1) 
	s1 = wb.add_sheet(sheet_name)

	s1.write(1,0,'Task-1')
	s1.write(11,0,'Task-2')
	s1.write(21,0,'Task-3')
	s1.write(31,0,'Task-4')
	...
	for i in range(len(energy_pa)):		
		...
		q_table.append(np.random.rand(pa_size[i],9) * 0.001 - 0.001)  # of states x # of actions
	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			[current_agent_list, next_agent_list] = simul_upt(pa2ts[which_pd], pa_s[which_pd], pa_t[which_pd], act_num[which_pd], real_action, real_agent_s)
			...
			for i in range(len(current_agent_list)):
				current_q = q_table[which_pd][current_agent_list[i], real_action]		
				max_future_q = np.amax(q_table[which_pd][next_agent_list[i], :])                                          # Find the max future q 	
				new_q        = (1 - LEARN_RATE) * current_q + LEARN_RATE * (rew_obs + DISCOUNT * max_future_q)
				q_table[which_pd][current_agent_list[i], real_action] = new_q 
			
			for i in range(len(energy_pa)):
				if i != which_pd:
					# hypo_agent_s = agent_s[i]
					# agent_s[i] = agent_upt[i].index(pa2ts[which_pd][agent_s[which_pd]])
					# current_q = q_table[i][hypo_agent_s, real_action]
					# max_future_q = np.amax(q_table[i][agent_s[i], :]) 
					# new_q        = (1 - LEARN_RATE) * current_q + LEARN_RATE * (rew_obs + DISCOUNT * max_future_q) # Calculate the new q value
					# q_table[i][hypo_agent_s, real_action] = new_q    # Update the table				
	                
					hypo_agent_s = agent_s[i]
					agent_s[i] = agent_upt[i].index(pa2ts[which_pd][agent_s[which_pd]])
					[current_agent_list, next_agent_list] = simul_upt(pa2ts[i], pa_s[i], pa_t[i], act_num[i], real_action, hypo_agent_s)
					for ind in range(len(current_agent_list)):
						current_q = q_table[i][current_agent_list[ind], real_action]		
						max_future_q = np.amax(q_table[i][next_agent_list[ind], :])                                          # Find the max future q 	
						new_q        = (1 - LEARN_RATE) * current_q + LEARN_RATE * (rew_obs + DISCOUNT * max_future_q)
						q_table[which_pd][current_agent_list[ind], real_action] = new_q 
		...
		if (episode+1) % 10000 == 0:
			...
			for ind in range(len(energy_pa)):
				...
				s1.write(ind*10+inx,1,len(ep_per_task[ind]))
				s1.write(ind*10+inx,2,avg_per_task)
	...
	filename = 't'+str(test_n+1) +'.xls'
	filename = 'time_not_included.xls'
	wb.save(filename)
...
if __name__ == '__main__':
	...
	ep_len = 19 # Episode length
	m = 6       # of rows
	n = 6       # of columns  8
	h = 1       # height set to 1 for 2D case
	init_states    = [(0,0,0)]                                         # Specify initial states and obstacles (row,column,altitude/height)
	obstacles = [(3,2,0)]    # Indexed from (0,0,0) which is the upper left corner at ground height
	pick_up_state = []
	delivery_state = []
	pick_up_state.append([(5,0,0)])                                      # 
	#pick_up_state.append([(0,7,0)])                                         # 
	#pick_up_state.append([(5,13,0)])                                         # 
	...
	for ind_p in range(len(pick_up_state)):
		for ind in range(len(delivery_state)):
			[i_s_i, pa_i, pa_s_i, pa_t_i, pa2ts_i, energy_pa_i, rewards_pa_i, pick_up_i, delivery_i] = prep_for_learning(ep_len, m, n, h, init_states, obstacles, pick_up_state[ind_p], delivery_state[ind], rewards, rew_val, custom_flag, custom_task)
			i_s.append(i_s_i)
	...
	N_EPISODES = 4000000      # of episodes
	SHOW_EVERY = 25000        # Print out the info at every ... episode
	LEARN_RATE = 0.1
	DISCOUNT   = 0.95
	EPS_DECAY  = 1 #0.9995 #0.9993
	epsilon    = 0.1 
	eps_unc    = 0.05 # Uncertainity in actions
	Pr_des     = 0.75
	...
	if thr_fun < Pr_des:
		...
		for test_n in range(1):
			opt_pol = Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states, pick_up, delivery,test_n )
