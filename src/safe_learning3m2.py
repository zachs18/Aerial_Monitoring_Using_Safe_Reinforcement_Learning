...
def Q_Learning(Pr_des, eps_unc, N_EPISODES, SHOW_EVERY, LEARN_RATE, DISCOUNT, EPS_DECAY, epsilon, i_s, pa, energy_pa, pa2ts, pa_s, pa_t, act_num, possible_acts, possible_next_states,  pick_up, delivery):
	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			rew_obs      = np.random.binomial(1, 1-rew_uncertainity) * rewards_pa[which_pd][agent_s[which_pd]]                 # Observe the rewards of the next state
			new_q        = (1 - LEARN_RATE) * current_q + LEARN_RATE * (rew_obs + DISCOUNT * max_future_q)
			q_table[which_pd][s_a[0], s_a[1]] = new_q
			ep_rew += rew_obs

			# print("Action : " + str(action))
			# print(pa2ts[which_pd][agent_s[which_pd]])
