	...
	for episode in range(N_EPISODES):
		...
		for t_ep in range(ep_len):
			...
			if np.random.uniform() > epsilon:                              # Exploit
				possible_qs = q_table[which_pd][agent_s,possible_acts_copy[agent_s]] # Possible Q values for each action
			...
			if np.random.uniform() < eps_unc:
				...
				current_q = q_table[which_pd][agent_s,action]                             # (save the current q for the q_table update later on)
...
if __name__ == '__main__':
	...
	for ind_p in range(len(pick_up_state)):
		for ind in range(len(delivery_state)):
			[i_s, pa_i, pa_s_i, pa_t_i, pa2ts_i, energy_pa_i, rewards_pa_i, pick_up_i] = prep_for_learning(ep_len, m, n, h, init_states, obstacles, pick_up_state[ind_p], delivery_state[ind], rewards, rew_val, custom_flag, custom_task)
			pa.append(pa_i)
